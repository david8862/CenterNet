...
class RoboRock(data.Dataset):
  ...
  def __init__(self, opt, split):
    super(RoboRock, self).__init__()
    self.data_dir = os.path.join(opt.data_dir, 'roborock')
    self.img_dir = os.path.join(self.data_dir, 'images')
    _ann_name = {'train': '2007_train', 'val': '2007_val', 'test': '2007_test'}
    self.annot_path = os.path.join(
      self.data_dir, 'annotations', 
      'roborock_{}.json').format(_ann_name[split])
    self.max_objs = 50
    self.class_name = ['__background__', "bar stool", "shoe", "pet feces", "wire"]
    ...
    print('==> initializing roborock {} data.'.format(_ann_name[split]))
